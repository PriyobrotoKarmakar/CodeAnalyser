...
import { BarChart3, Send, Code2, Eye, Edit3 } from 'lucide-react';
import { Card, Button, LoadingSpinner } from '../styles/components.js';
import { apiService } from '../services/api.js';
import RobotHead from './RobotHead.jsx';
import MarkdownRenderer from './MarkdownRenderer.jsx';
import ComplexityGraph from './ComplexityGraph.jsx';
import { useTheme } from '../contexts/ThemeContext.jsx';
import { extractComplexities, hasComplexityInfo } from '../utils/complexityExtractor.js';
...
const AnalysisPanel = styled.div`
  /* Analysis content takes full width on mobile, left side on desktop */
`;

const GraphPanel = styled.div`
  /* Graph takes full width on mobile, right side on desktop */
`;
...
const ComplexitySection = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error('Complexity API Error:', err);
              ...
              <AnalysisPanel>
                <ResultContainer>
                  {error ? (
                    ...
                    <EmptyState>
                      Submit your code to see the complexity analysis here...
                    </EmptyState>
                  )}
                </ResultContainer>
              </AnalysisPanel>

              {result && hasComplexityInfo(result) && (
                <GraphPanel>
                  <ComplexityGraph 
                    {...extractComplexities(result)}
                    theme={theme}
                  />
                </GraphPanel>
              )}
