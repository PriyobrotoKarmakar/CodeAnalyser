...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            print(f"Processing request for path: '{path}'")  # Debug log
            ...
            # Initialize Gemini (get API key from environment - reload to ensure fresh value)
            try:
                from dotenv import load_dotenv
                load_dotenv('../.env', override=True)  # Force reload with override
                load_dotenv('.env', override=True)     # Try both locations
            except ImportError:
                pass
            ...
            print(f"Using API key in request: {api_key[:10]}...")  # Debug actual key used in request
    ...
    def _handle_graph_data(self, client, data):
        ...
        graph_data = self._generate_mathematical_data(complexity_type, max_input_size)
    ...
    def _generate_mathematical_data(self, complexity_type, max_input_size):
